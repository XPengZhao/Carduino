...
double Setpoint, Output;                               //角度DIP设定点，输入，输出
...
double setp0 = 0, dpwm = 0, dl = 0, pwm1 = 0, pwm2 = 0; //角度平衡点，PWM差，死区，PWM1，PWM2
float value;
...
void angleout()
{
  Output = kp * (angle + angle0) + kd * Gyro_x;//PD 角度环控制
}
//////////////////角度PD////////////////////
...
void pwma()
{
  ...
  pwm1 = -Output - Outputs - turnoutput; //Left电机PWM输出值
...
//  }
  //悬空识别
    if (zhitaicount > 10 & turnl == 0 & turnr == 0 & spinl == 0 & spinr == 0 && front == 0 && back == 0) //50ms检测是否悬空
    {
  
      if  ((balancecar.pulseright + balancecar.pulseleft ) > -2 && (balancecar.pulseright + balancecar.pulseleft) < 2)
      {
        flag1 = 1;
      }
      if (flag1 == 1 )
      ...
      {
        //flag=0;
        // BLANACECAR.pulseleft = BLANACECAR.pulseright = 0;
        pwm1 = 0;
        pwm2 = 0;
    ...
    if (zhitaicount > 100)
    {
      ...
      if (flag4 == 1)
      {
        if (balancecar.pulseright == 0 && balancecar.pulseleft == 0)
        {
  
          flag4 = 2;
          balancecar.pulseright = balancecar.pulseleft = 0;
      ...
      if (flag4 == 2)
      {
        if ((balancecar.pulseleft < 2 ) && ( balancecar.pulseright < 2) && accelz > 1500) //&&BLANACECAR.pulseleft<-2&&BLANACECAR.pulseright<-2 )
        {
          stopl = stopr = 0;
          pwm1 = pwm2 = 0;
